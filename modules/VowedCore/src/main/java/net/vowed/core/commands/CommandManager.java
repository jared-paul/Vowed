...
public class CommandManager implements CommandExecutor
{
    ...
    private void execute(CommandSender sender, CommandInfo commandInfo) throws CommandException
    {
        ...
        args.remove(commandInfo.subRoot);
    ...
    {
        String commandName = commandInfo.root + " " + commandInfo.subRoot;
        ...
        {
            Vowed.LOG.warning("COMMAND ERROR: Failed to execute command");
            e.printStackTrace();
        }
    }
    ...
    {
        String combinedRoot = (commandInfo.root + " " + commandInfo.subRoot).toLowerCase();
        String command = combinedRoot + " " + commandInfo.initializer.toLowerCase();

        return commands.containsKey(command) || commands.containsKey(combinedRoot + " *");
    }

    public boolean hasRoot(CommandInfo command)
    {
        String subRoot = command.subRoot.toLowerCase();
        boolean hasRoot = false;
        for (String root : commands.keySet())
        {
            String realRoot = root.split(" ")[1];

            if (realRoot.equals(subRoot))
            {
                hasRoot = true;
            }
        }
    ...
    private boolean suggestClosestModifier(CommandSender sender, CommandInfo commandInfo)
    {
        ...
        if (!closest.isEmpty())
        {
            ...
            sender.sendMessage(ChatColor.RED + "Did you mean " + "\"" + ChatColor.YELLOW + "/" + commandInfo.root + " " + commandInfo.subRoot + " " + closest + ChatColor.RED + "\"" + " ?");
            return true;
        }
        return false;
    }
    ...
    public String getClosestCommandModifier(CommandInfo commandInfo)
    {
        ...
        String command = (commandInfo.root + " " + commandInfo.subRoot).toLowerCase();
        String closest = "";
        for (String cmd : commands.keySet())
        {
            String[] split = cmd.split(" ");

            if (!(split[0] + " " + split[1]).equals(command) || split.length <= 1)
            {
                continue;
            }
            ...
            if (commandInfo.nested)
            {
                try
                {
                    String lookingFor = split[2] + " " + split[3];

                    int distance = getLevenshteinDistance(commandInfo.initializer, lookingFor);
                    if (minDist > distance)
                    {
                        minDist = distance;
                        closest = split[2] + " " + split[3];
                    }
                }
                catch (IndexOutOfBoundsException exception)
                {
                    int distance = getLevenshteinDistance(commandInfo.initializer, split[2]);
                    if (minDist > distance)
                    {
                        minDist = distance;
                        closest = split[2];
    ...
    {
        int minDist = Integer.MAX_VALUE;
        String subRoot = command.subRoot;
        String closest = "";
        for (String cmd : commands.keySet())
        {
            Vowed.LOG.debug("finding root");
    ...
    public boolean onCommand(CommandSender sender, org.bukkit.command.Command command, String cmdName, String[] args)
    {
        ...
        String subRoot = args.length > 0 ? args[0] : "";
        String initializer = args.length > 1 ? args[1] : "";
        String nestedInitializer = args.length > 2 ? args[2] : "";

        Vowed.LOG.debug(nestedInitializer);


        Object[] methodArgs = {sender};

        CommandInfo nestedCommandInfo = new CommandInfo(root, subRoot, initializer + " " + nestedInitializer, args, methodArgs);
        nestedCommandInfo.nested = true;


        List<String> newArgs = Lists.newArrayList(args);
        newArgs.remove(subRoot);
        ...
        {
            Vowed.LOG.debug("root not there");
        ...
        {
            CommandInfo commandInfo = new CommandInfo(root, subRoot, initializer, args, methodArgs);

            if (!hasCommand(commandInfo))
            {
                Vowed.LOG.debug("command not there");
