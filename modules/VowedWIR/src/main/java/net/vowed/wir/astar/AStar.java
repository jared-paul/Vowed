...
public class AStar
{
    ...
    public AStar(Location start, Location end, int range) throws InvalidPathException
    {
        ...
        boolean isStart, isEnd = true;

        if (!(isStart = this.isLocationWalkable(start)) || !(isEnd = this.isLocationWalkable(end)))
        {
            throw new InvalidPathException(isStart, isEnd);
        }
