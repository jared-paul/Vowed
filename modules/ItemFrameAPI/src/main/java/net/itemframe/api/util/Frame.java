...
import net.itemframe.api.render.ImageRenderer;
import net.itemframe.api.render.TextRenderer;
...
import org.bukkit.craftbukkit.v1_11_R1.map.RenderData;
...
public class Frame {
    ...
    private void sendItemMeta(Player player) {
        ...
        if (this.cachedItemPacket == null) {
            ...
            Utils.setPrivateField(nmsItem, "count", 1);
            nmsItem.a(entity);

            DataWatcher watcher = new DataWatcher(entity);
            watcher.register(DataWatcherRegistry.c.a(8), nmsItem);
            watcher.register(DataWatcherRegistry.c.a(3), (byte)0);
           // watcher.update(8);
    ...
    private void sendMapData(Player player) {
        if (this.cachedDataPacket == null) {
            ...
            this.cachedDataPacket = new PacketPlayOutMap(this.getMapId(), (byte) 3, true, new ArrayList<MapIcon>(), data.buffer, 0, 0, 128, 128);
        }
